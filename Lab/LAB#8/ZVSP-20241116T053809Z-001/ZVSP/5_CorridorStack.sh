...
#except_first_trace=5; it depends on the direction of the zvsp (sign of depth) otherwise start with i=1 (not 5) and open the flat up in the last 5 traces by setting except_last_trace to 5
...
awk -v corridor=$corridor -v last_rec=$last_rec \
	'BEGIN {i=1}{if (i>5 && i<=last_rec){print (($1*2)+corridor), $2} else {print 9,$2} i++}' $tt_picks \
	| awk '{ printf "%4f %d\n", $1, $2 }' > $tt_picks.inside 
...
sumute < $input_decon_enh_up key=tracr nmute=$nrec xfile=xfile.bin tfile=tfile.outside.bin mode=0 \
	...
	| suxwigb perc=99 title="Enhanced Deconvolved Upgoing"  key=gelev \
	curve=$tt_picks.inside,$tt_picks.outside npair=$nrec,$nrec curvecolor=red label1='Time (s)' label2='Depth (ft)'&

#mute, change word, stack
sumute < $input_decon_enh_up key=tracr nmute=$nrec xfile=xfile.bin tfile=tfile.outside.bin mode=0 | sumute  key=tracr nmute=$nrec xfile=xfile.bin tfile=tfile.inside.bin mode=1 | sustack repeat=1 key=ep normpow=1.0 > $output_cstack

#display	
suwind <  $output_cstack tmin=$tmin tmax=$tmax \
	| suxwigb perc=99 xbox=610 wbox=200 label1='Time (s)' title='Corridor Stack' &
