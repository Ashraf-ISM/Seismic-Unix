...
#input
input_dn=velf_dn.su
output_pef_dn=Test_velf_dn.supef_dn.su

#set parameter
minlag=0.012 #s
maxlag=0.4 #s
pnoise=0.0005
bpf=8,12,80,90 #4 points bandpass specification
...
suxwigb < $input_dn.align.tmp perc=99 title="Testing Downgoing Before PEF Decon" \
	label1="Time (sec)" label2="Level Number"  wbox=550 xbox=10&
suxwigb < $output_pef_dn.align.tmp perc=99 title="Testing Downgoing After PEF Decon (minlag:$minlag s, maxlag:$maxlag s)"  \
	label1="Time (sec)" label2="Level Number"  wbox=550 xbox=570&
